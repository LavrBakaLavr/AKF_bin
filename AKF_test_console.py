...
f = open('akf_bin_47.txt', 'a')
f.write("Старт вычислений. Начало: 47, шаг: 4, индекс GPU: 0")
f.close()
...
for k in range(line_lenght_min, line_lenght_max + 1, 4): 
    ...
    for j in range(batch_pow // 4, batch_pow // 4 * 3):
        ...
        if k/optimal_amp >= best :
            ...
            print ('For {} in batch: {}/{}, no better than {} - {} sec'.format(
                k, j+1, batch_pow, best, np.round((time.time() - start_time), 2)), end='')
